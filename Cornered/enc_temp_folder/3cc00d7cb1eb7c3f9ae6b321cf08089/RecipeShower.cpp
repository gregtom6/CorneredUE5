...
void ARecipeShower::MoveToNextRow(FVector& PositionForGeneration) {
	PositionForGeneration = FVector(StartingTransform->GetComponentLocation().X, StartingTransform->GetComponentLocation().Y, StartingTransform->GetComponentLocation().Z - VerticalGapSize);
}
