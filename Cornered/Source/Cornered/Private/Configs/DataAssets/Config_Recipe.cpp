...
TSubclassOf<AIngredient> UConfig_Recipe::GetResultItem(TArray<FItemData> detectedItems) const {
	...
	for (int i = 0; i < RecipeEntries.Num(); i++)
	{
		...
		for (int j = 0; j < RecipeEntries[i].Items.Num(); j++) {
			...
			if (GetOccurrenceNumber(RecipeEntries[i].Items, RecipeEntries[i].Items[j].ItemType, RecipeEntries[i].Items[j].ItemState) == 
				GetOccurrenceNumber(detectedItems, RecipeEntries[i].Items[j].ItemType, RecipeEntries[i].Items[j].ItemState)) {
				equalElements += 1;
			}
		}
...
TArray<FMaterialArray> UConfig_Recipe::GetRadiatingMaterialsOfAllRecipes(TArray<FMaterialArray>& EffectMaterials, UCorneredGameInstance* GameInstance) {
	...
	for (int j = 0; j < RecipeEntries.Num(); j++) {
		...
		for (int32 k = 0; k < recipeEntry.Items.Num(); k++) {
			...
			FillUpMaterials(recipeEntry.Items[k].ItemType, recipeEntry.Items[k].ItemState, Materials[i], ItemStateMaterials[i]);
		}

		FillUpMaterials(recipeEntry.Key, EItemState::Normal, Materials[i], ItemStateMaterials[i]);
...
void UConfig_Recipe::FillUpMaterials(EItemType ItemType,EItemState ItemState, FMaterialArray& Material,FMaterialArray& ItemStateMaterial ) {
	UMaterialInterface* mat = GetRadiatingMaterial(ItemType);
	Material.Materials.Add(mat);
	UMaterialInterface* stateMat = GetRadiatingEffectMaterial(ItemState);
	ItemStateMaterial.Materials.Add(stateMat);
}
