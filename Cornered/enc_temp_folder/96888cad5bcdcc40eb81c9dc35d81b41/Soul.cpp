...
void ASoul::Tick(float DeltaTime)
{
	...
	if (MoveState == ESoulMoveState::MoveUpwards) {
		...
		//int32 indexOfClosestSplinePoint = SoulRoute->GetIndexOfSplinePoint(ClosestSplinePoint);
		//FSplinePoint BetterSplinePoint = SoulRoute->GetSplinePoint(indexOfClosestSplinePoint);
		...
		Movable->SetWorldLocation(FMath::Lerp(StartingPosition, TargetPosition, ToSplineProgress));
		ToSplineProgress += SoulConfig->SpeedTowardsSpline * DeltaTime;
		...
		//int index = SoulRoute->GetIndexOfSplinePoint(TargetPosition);
		//float SplineDistance = SoulRoute->GetDistanceAlongSplineAtSplinePoint(index)
		//Movable->SetWorldLocation(Position);
		//ToSplineProgress += SoulConfig->SpeedAlongSpline * DeltaTime;
		Movable->SetWorldLocation(TargetPosition);
		/*
		if (ToSplineProgress >= 1.0f) {
